...
async function savePracticeSettings() {
  ...
  try {
    ...
    await apiPost("/api/practice-settings", {
...
async function previewPracticeSchedule() {
  ...
  let fieldCount = Number(qs("practice_field_count")?.value || 1);
  ...
  try {
    ...
    const data = await apiPost("/api/practice-matches/preview", {
...
async function createPracticeMatch() {
  ...
  try {
    ...
    await apiPost("/api/practice-matches", {
...
async function updatePracticeMatch(matchId, updates) {
  try {
    await apiPost(`/api/practice-matches/${matchId}`, updates);
...
async function deletePracticeMatch(matchId) {
  try {
    await apiDelete(`/api/practice-matches/${matchId}`);
    if (res.status === 403) {
...
async function bulkUpdatePracticeMatches() {
  ...
  try {
    ...
    const data = await apiPost("/api/practice-matches/bulk-update", {
...
function printPracticeSchedule() {
  ...
  const breaksSummary = qs("practice_breaks_summary");
  if (!printArea) {
  ...
  fetch("/api/event")
    ...
    .then(event => {
      ...
      const breaksHtml = breaksSummary ? breaksSummary.innerHTML : "";
      printWindow.document.write(`
        ...
        <head>
          ...
          <style>
            ...
            .breaks-summary { margin-bottom: 16px; padding: 8px 10px; border: 1px solid #ddd; border-radius: 6px; background: #fafafa; }
          </style>
        </head>
        <body>
          ...
          ${breaksHtml ? `<div class="breaks-summary">${breaksHtml}</div>` : ""}
          ${printArea.innerHTML}
        </body>
        </html>
      `);
...
async function renderPracticeGrid() {
  ...
  try {
    const matches = await apiGet("/api/practice-matches", { date: selectedDate });
    ...
    console.error("Render practice grid error:", err);
    gridContainer.textContent = "Takvim yüklenirken hata oluştu.";
  }
}
...
async function generatePracticeMatches() {
  ...
  try {
    ...
    const data = await apiPost("/api/practice-matches/generate", {
      start_date: startDate,
      start_time: startTime,
      field_count: fieldCount,
      teams_per_alliance: teamsPerAlliance,
